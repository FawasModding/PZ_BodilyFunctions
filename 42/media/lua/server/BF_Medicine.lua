function OnEat_AntiDiarrhealPill(food, player, percent)
    ...
    local currentBowels = BathroomFunctions.GetDefecateValue()
    local reduction = bowelsMaxValue * 0.1 * percent -- Reduce bowel value by 10% of max per pill
    BathroomFunctions.SetDefecateValue(math.max(0, currentBowels - reduction))
...
function OnEat_LaxativePill(food, player, percent)
    ...
    BathroomFunctions.SetDefecateValue(math.min(bowelsMaxValue, currentBowels + increase))
...
function OnEat_DiureticPill(food, player, percent)
    ...
    local currentBladder = BathroomFunctions.GetUrinateValue()
    local increase = bladderMaxValue * 0.2 * percent -- Increase bladder value by 20% of max per pill
    BathroomFunctions.SetUrinateValue(math.min(bladderMaxValue, currentBladder + increase))
...
function OnEat_AnticholinergicPill(food, player, percent)
    ...
    BathroomFunctions.SetUrinateValue(math.max(0, currentBladder - reduction))
